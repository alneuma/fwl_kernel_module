...
#define PLOG_DRIVER_NAME "periodic_logger"
#define PLOG_LOG_INTERVAL HZ
#define PLOG_MSG_BUFSIZE 8
 ...
 * 2. plog_data
 */
static DEFINE_MUTEX(plog_control);
static DEFINE_MUTEX(plog_data);
static dev_t devt;
static struct cdev periodic_logger;
static struct class *cls;
static struct delayed_work plog_work;
static unsigned long next_log;
static char message_buf[PLOG_MSG_BUFSIZE];
 ...
 */
static void plog_work_handler(struct work_struct *work)
{
	struct delayed_work *dwork;
	unsigned long delay;

	mutex_lock(&plog_data);

	if (!message_size) {
		mutex_unlock(&plog_data);
		return;
	}
 ...
 * plog_schedule_work()
 */
static void plog_schedule_work(void)
{
	if (schedule_delayed_work(&plog_work, PLOG_LOG_INTERVAL)) {
		next_log = jiffies + PLOG_LOG_INTERVAL;
 ...
 * plog_open()
 */
static int plog_open(struct inode *inode, struct file *filp)
{
	pr_debug("called\n");
	return 0;
}
 ...
 * plog_release()
 */
static int plog_release(struct inode *inode, struct file *filp)
{
	pr_debug("called\n");
	return 0;
}
 ...
 */
static ssize_t plog_write(struct file *filp, const char __user *buf,
			  size_t count, loff_t *f_pos)
{
	size_t copy_size = min(count, PLOG_MSG_BUFSIZE);
	char tmp_buf[PLOG_MSG_BUFSIZE];
 ...
 */
static ssize_t plog_read(struct file *filp, char __user *buf, size_t count,
			 loff_t *f_pos)
{
	pr_debug("called\n");
	...
	(void)cancel_delayed_work_sync(&plog_work);
	mutex_unlock(&plog_control);

	return 0;
}

static const struct file_operations plog_ops = {
	.owner = THIS_MODULE,
	.open = plog_open,
...
static int __init plog_init(void)
{
	...
	INIT_DELAYED_WORK(&plog_work, plog_work_handler);

	ret = alloc_chrdev_region(&devt, 0, 1, PLOG_DRIVER_NAME);
	if (ret) {
		pr_err("failed to initialize %s: alloc_chrdev_region(): %d\n",
		       PLOG_DRIVER_NAME, ret);
		goto err_alloc_chrdev_region;
	}

	cdev_init(&periodic_logger, &plog_ops);
	ret = cdev_add(&periodic_logger, devt, 1);
	if (ret) {
	...
	cls = class_create(PLOG_DRIVER_NAME);
	if (IS_ERR(cls)) {
		ret = PTR_ERR(cls);
	...
	dev_ptr = device_create(cls, NULL, devt, NULL, PLOG_DRIVER_NAME);
	if (IS_ERR(dev_ptr)) {
		ret = PTR_ERR(dev_ptr);
