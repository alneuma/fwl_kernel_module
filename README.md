...
![C](https://img.shields.io/badge/C-5C6D7E?style=flat-square&logo=c&logoColor=A8B9CC) ![Linux kernel 6.12](https://img.shields.io/badge/-Linux%20kernel%206.12-2C6BED?style=flat-square&logo=linux&logoColor=white)
...
What looks deceptively simple on the surface turned into a complex problem, once taken seriously:
...
([chunk_reader](https://github.com/alneuma/chardev_test_utils) is a small C program that reads from a file with specified read buffer sizes in specified intervals and prints the result to stdout.)
...
experiments/           experiments I used to explore kernel concepts
  ...
  periodic_logger/     delayed-work logging + fighting interval-drift, no word queue yet
...
- Making multi-stage writes commit under exclusive lock
...
The central data structure is a queue of words implemented as a list. Writing to the device appends words to this queue. Reading from the device returns the current content of the queue. For this purpose the content is formatted as a stream of words separated by single-byte separators. As long as there are enqueued words, once a second a work function removes the first word from the queue and logs it. There are five concurrent entry points contending for shared state:
...
| `work_handler()` | The work function writes to the queue, i.e. removes words. Also accesses device wide memory accounting. |

- To account for read position and unfinished words, OFDs save their per-OFD state between accesses.
- To keep track of read position a `struct fwl_cursor` variable is employed, which references word nodes and whose validity can be verified by a node indexing system.
...
- Changes to the queue through write are first created and validated within a transaction object, before they get committed to the queue under a single lock-guarded update.
- A self-rescheduling delayed work item once per second asynchronously removes words from the queue and logs them.
...
- any byte in the set defined by the kernel's isspace() function
...
There is a limit on long-lived memory, currently hard-coded as *1 MB* [(which is not fully accurate)](#memory-accounting). This is memory used by the queue itself and memory used to save per-OFD state.
While `release()` can commit already claimed memory to the queue, only `write()` and `open()` can make the device claim more long-lived memory. If they try to do so but no more memory is available, `-ENOSPC` is returned.
...
There are a couple of general considerations for writing code in kernel-space:
...
3. There is no *libc*, we cannot use system calls, the kernel provides them. All the help at our disposal comes from the internal Linux kernel API.
...
| 2 | `rw_sem_logging` | read/write semaphore protecting device-wide shared data in situations where the only disruptive interference to readers could come from the word logging and dequeuing mechanism |
...
| `mem_used` | long-lived dynamic memory occupied by the device, allocator overhead not taken into account |
...
| `struct fwl_transaction_write` | this is used to prepare and validate data received by write() before it is committed to the device's long-lived state |

### Transactions
When a write happens, data flows from user-space to the device. Many things can go wrong. Before any changes are committed to the device's long-lived state, a transaction object is prepared and validated. If any non-recoverable error happens, the transaction object is dropped and the device's long-lived state stays untouched. If the transaction is safe to be committed, this happens as a single update, while `rw_sem_user` is held exclusively.
...
Logging is implemented by self-rescheduling delayed work items and a single work function.

#### Two states
There are two relevant states: `queue empty` and `queue non-empty`.

##### queue empty
As long as the queue is empty, no new work items are scheduled. If the work function is currently executing, then it has already reached a state in which it has determined that it won't self-reschedule.

##### queue non-empty
When the queue is non-empty a work item is already scheduled or about to be scheduled. The periodically (1 second intervals) executed work function deletes the first word in the queue and then reschedules the work item if `queue non-empty` still persists. Interval drift is prevented by the work function calculating the ideal execution time of its next call relative to the ideal execution time of the current. The ideal execution time then is compared to the current time to determine for how many `jiffies` in the future the next call should be scheduled.

##### transitions

Every transition from `queue empty` to `queue non-empty` is fully controlled by `write()` and `release()`. Lock holding prevents race conditions.

Every transition from `queue non-empty` to `queue empty` is fully controlled by the work function and also happens while an exclusive lock is held.

Because `read()` temporarily releases `rw_sem_user` when copying from its internal buffer to the user's read buffer, there is a possibility that the work function removes a word from the queue during this time. Under certain edge cases this could invalidate the read cursor. To prevent this `rw_sem_logging` is used. As logging only happens once a second there should be very little lock contention.
...
So there are *4294967295 - 32537631 = 4262429664* seconds left during which one *1 byte* word per second needs to be enqueued until indices are exhausted.
...
The current strategy for memory accounting enforces a device-wide limit on dynamic memory claimed by the device for maintaining long-lived state. It accounts for queue nodes and per-OFD state.
...
[chunk_reader](https://github.com/alneuma/chardev_test_utils): reads with fixed buffer sizes from a file and prints to stdout. The buffer size, as well as a delay between reads can be passed as command line arguments. This was most helpful when testing the read() during concurrent modification of the word queue by the logging mechanism.
...
| Multiple concurrent accesses via the same OFD | Yes | No |
...
- How can a multi-stage operation fail without corrupting long-lived state?
